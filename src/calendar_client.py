        ...
        if not creds or not creds.valid:
            ...
            if not creds:
                ...
                try:
                    ...
                    # Use a fixed port (8080) to ensure redirect URI matches Google Cloud Console configuration
                    # If this port is in use, try alternative ports or use console flow
                    try:
                        creds = flow.run_local_server(port=8080, open_browser=True)
                    except OSError:
                        # Port 8080 is in use, try alternative port
                        try:
                            creds = flow.run_local_server(port=8081, open_browser=True)
                        except OSError:
                            # If local server fails, fall back to console flow (requires manual copy-paste)
                            sys.stderr.write(
                                "Local server ports unavailable. Using console flow.\n"
                                "Please visit the URL shown and paste the authorization code.\n"
                            )
                            creds = flow.run_console()
                except Exception as e:
                    # If redirect_uri_mismatch error, provide helpful instructions
                    error_msg = str(e)
                    if "redirect_uri_mismatch" in error_msg.lower() or "400" in error_msg:
                        sys.stderr.write(
                            ...
                            "="*70 + "\n"
                            "To fix this error, you need to add the redirect URI to your Google Cloud Console:\n\n"
