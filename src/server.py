...
# --- Monday.com Placeholder (FIXED NAME ERROR) ---
# NOTE: The actual MondayClient class and MONDAY_AVAILABLE check is missing
# from your repository files (e.g., monday_client.py), so we use placeholders
# to prevent the NameError and skip initialization gracefully.

try:
    # Attempt to import the client if it exists (assuming a file named monday_client.py)
    from .monday_client import MondayClient
    MONDAY_AVAILABLE = True
except ImportError:
    # Define placeholder classes/variables to prevent NameError
    class MondayClient:
        def __init__(self):
            raise ImportError("MondayClient is not implemented or imported.")
        def get_boards(self, limit: int):
            return []
        def get_board_items(self, board_id: int, limit: int):
            return []
...
monday_client: Optional[Any] = None # <-- FIXED: Used Optional[Any] instead of Optional[MondayClient]
...
def initialize_monday_client():
    """Initialize the Monday.com client."""
    global monday_client
    if monday_client is None:
        if not MONDAY_AVAILABLE:
            raise RuntimeError(
                "Monday.com client not available. MondayClient class not found."
            )
        try:
            monday_client = MondayClient()
        except Exception as e:
            raise RuntimeError(f"Failed to initialize Monday.com client: {e}")
...
@mcp.tool()
def get_monday_boards(limit: int = 20) -> str:
    """
    Get all Monday.com boards that you have access to.
    
    Args:
        limit: Maximum number of boards to return (default: 20)
    
    Returns:
        Formatted list of boards with their IDs, names, and item counts
    """
    try:
        initialize_monday_client()
        
        boards = monday_client.get_boards(limit=limit)
        
        if not boards:
            return "No Monday.com boards found."
        
        result_parts = [f"Found {len(boards)} Monday.com board(s):\n"]
        ...
        for i, board in enumerate(boards, 1):
            ...
            result_parts.append(f"      State: {state} | Items: {items_count} | Type: {board_kind}")
            
            groups = board.get('groups', [])
            if groups:
                group_names = [g.get('title', '') for g in groups[:5]]
                result_parts.append(f"      Groups: {', '.join(group_names)}")
        ...
        logger.error(f"Error in get_monday_boards: {e}", exc_info=True)
        # Check for specific MondayClient import error
        if isinstance(e, RuntimeError) and "MondayClient class not found" in str(e):
             return "Error fetching Monday.com boards: Monday.com client is not configured. Please check MONDAY_AVAILABLE status."
        return f"Error fetching Monday.com boards: {str(e)}"


@mcp.tool()
def get_monday_board_items(board_id: int, limit: int = 20) -> str:
    """
    Get items from a specific Monday.com board.
    
    Args:
        board_id: ID of the board (get board IDs using get_monday_boards)
        limit: Maximum number of items to return (default: 20)
    
    Returns:
        Formatted list of items with their details and updates
    """
    try:
        initialize_monday_client()
        ...
        if not items:
            return f"No items found in board {board_id}."
        
        result_parts = [f"Found {len(items)} item(s) in board {board_id}:\n"]
        ...
        for i, item in enumerate(items, 1):
            ...
            result_parts.append(f"   {i}. {name} (ID: {item_id})")
            ...
            if column_values:
                cols = []
                for col in column_values[:5]:
                    col_text = col.get('text', '')
                    if col_text:
                        cols.append(col_text)
                if cols:
                    result_parts.append(f"      Values: {', '.join(cols)}")
            
            # Show recent updates
            updates = item.get('updates', [])
            if updates:
                result_parts.append(f"      Recent updates: {len(updates)}")
        ...
        logger.error(f"Error in get_monday_board_items: {e}", exc_info=True)
        # Check for specific MondayClient import error
        if isinstance(e, RuntimeError) and "MondayClient class not found" in str(e):
             return "Error fetching Monday.com items: Monday.com client is not configured. Please check MONDAY_AVAILABLE status."
        return f"Error fetching Monday.com board items: {str(e)}"


@mcp.tool()
def chat(message: str, include_calendar_context: bool = True, include_github_context: bool = False, include_monday_context: bool = False) -> str:
    """
    Chat with the AI assistant about your calendar, GitHub, and Monday.com. This is a conversational interface
    that uses Gemini AI to answer questions about your schedule, availability, events, GitHub activity, and project timelines.
    ...
    Args:
        ...
        include_monday_context: Whether to include Monday.com context in the response (default: False)
    ...
    try:
        ...
        # Detect Monday.com queries
        monday_keywords = ['monday', 'project', 'timeline', 'board', 'task', 'item', 'sprint', 'milestone', 'deadline', 'status', 'progress', 'workflow']
        if not include_monday_context:
            include_monday_context = any(keyword in message_lower for keyword in monday_keywords)
                    ...
                    try:
                        ...
                        if repo_match:
                            ...
                            if all_commits:
                                ...
                                github_context_parts.append("No recent commits found across tracked repositories.")
                    except Exception as e:
        ...
        # Fetch Monday.com context if requested
        if include_monday_context:
            try:
                initialize_monday_client()
                
                monday_context_parts = []
                
                # Get all boards
                try:
                    boards = monday_client.get_boards(limit=20)
                    if boards:
                        monday_context_parts.append(f"MONDAY.COM BOARDS ({len(boards)} total):")
                        monday_context_parts.append("-" * 50)
                        ...
                        for board in boards[:10]:  # Limit to 10 boards
                            ...
                            monday_context_parts.append(f"  - {name} (ID: {board_id})")
                            monday_context_parts.append(f"    Items: {items_count} | State: {state} | Type: {board_kind}")
                            
                            # Get items from this board if it's relevant or if board name matches query
                            if items_count > 0 and (any(keyword in message_lower for keyword in [name.lower(), 'item', 'task', 'timeline', 'project']) or items_count > 0):
                                try:
                                    items = monday_client.get_board_items(board_id, limit=15)
                                    if items:
                                        monday_context_parts.append(f"    Recent items ({len(items)} shown):")
                                        for item in items[:10]:  # Show up to 10 items
                                            ...
                                            column_values = item.get('column_values', [])
                                            col_texts = []
                                            for col in column_values[:3]:  # Show first 3 column values
                                                col_text = col.get('text', '')
                                                if col_text:
                                                    col_texts.append(col_text)
                                            
                                            item_line = f"      • {item_name} ({item_state})"
                                            if col_texts:
                                                item_line += f" - {', '.join(col_texts)}"
                                            monday_context_parts.append(item_line)
                                            
                                            # Show updates count if available
                                            updates = item.get('updates', [])
                                            if updates:
                                                monday_context_parts.append(f"        Updates: {len(updates)}")
                                except Exception as e:
                                    # Silently fail for individual boards
                                    pass
                            
                            monday_context_parts.append("")
                        
                        monday_context = "\n".join(monday_context_parts)
                    else:
                        monday_context = "No Monday.com boards found."
                except Exception as e:
                    error_msg = str(e)
                    if "401" in error_msg or "authorization" in error_msg.lower():
                        monday_context = (
                            "Note: Monday.com authorization failed. "
                            "Please check your MONDAY_API_TOKEN in the .env file. "
                            "Get a new token from: https://monday.com/mypage/settings/api"
                        )
                    else:
                        monday_context = f"Note: Could not fetch Monday.com boards: {error_msg}"
            except Exception as e:
                error_msg = str(e)
                if "401" in error_msg or "authorization" in error_msg.lower():
                    monday_context = (
                        "Note: Monday.com authorization failed. "
                        "Please verify your MONDAY_API_TOKEN is valid and active. "
                        "Get a new token from: https://monday.com/mypage/settings/api"
                    )
                else:
                    monday_context = f"Note: Monday.com client not available: {error_msg}"
        ...
        if calendar_context or github_context or monday_context or correlation_context:
            context_parts = []
            if calendar_context:
            ...
            if monday_context:
                context_parts.append(monday_context)
